...
public:
    TokenBucket(int64_t cap, int64_t r)
        : tokens_(cap), capacity_(cap), rate_(r),
    ...
    bool try_consume() {
        ...
        // 先补充这段时间新产生的令牌，再尝试消费
        tokens_ = std::min(capacity_, tokens_ + static_cast<int64_t>(elapsed * rate_));
        if (tokens_ > 0) {
            --tokens_;
            return true;
        }
...
```
1. Hash 分桶：将 10 亿 IP hash(IP) % 1000 → 1000 个小文件
   确保相同 IP 落在同一文件
...
**关键点：**
- **分片大小**：太小请求多 → 1MB 左右合适
- **校验**：每个分片 CRC 校验 → 防传输错误；全文件 MD5/SHA-256 → 完整性校验（MD5 性能好但碰撞脆弱，防篡改用 SHA-256）
...
> Raft 是强 Leader 的共识算法，核心机制包括：1）选主：节点超时后发起选举，获得多数票成为 Leader（term 递增）；2）日志复制：Leader 将操作通过 AppendEntries 复制到 Follower，多数确认后提交；3）安全性：通过 term 和 index 确保日志一致性，只有日志最新的节点才能当选 Leader（通过比较最后一条日志的 term 和 index，确保包含所有已提交日志）。
...
| 优化方法 | Tiered Compaction（减少数据重写） | Bloom Filter（快速排除不存在的 key） |
| 典型值 | LevelDB: 10-30x; RocksDB: 可优化到 3-5x | 使用 Bloom Filter 可将不存在 key 的点查询读放大降至 ~1 IO |
