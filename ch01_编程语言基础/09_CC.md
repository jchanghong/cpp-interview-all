...
* 引用
    * 指向常量的引用（reference to const），如 `const int &ref = a;`
    * 不存在"常量引用"（const reference 修饰引用自身），因为引用本身不是对象，不能用 const 修饰引用自身；但 `const T&` 是合法的，它表示"指向常量的引用"
...
底层实现原理与编译器相关，一般通过**虚基类指针**和**虚基类表**实现，每个虚继承的子类都有一个虚基类指针（占用一个指针的存储空间，32位系统4字节，64位系统8字节）和虚基类表（不占用类对象的存储空间）（需要强调的是，虚基类依旧会在子类里面存在拷贝，只是仅仅最多存在一份而已，并不是不在子类里面了）；当虚继承的子类被当做父类继承时，虚基类指针也会被继承。
