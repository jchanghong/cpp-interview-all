...
`char str1[] = "hello"`：字符数组可修改；`char *str2 = "hello"`：指向字符串常量（只读，修改为未定义行为）。// ⚠️ C++03 起字符串字面量类型为 `const char[N]`；`char*` 指向字面量的隐式转换在 C++03 中已 deprecated，C++11 起为 ill-formed；应使用 `const char*`。
...
`strcpy` 不检查目标缓冲区大小，可能造成缓冲区溢出。`strncpy` 可指定最大拷贝长度，但源串长度超 n 时不自动加 `\0`。
// C11 引入 `strncpy_s(dest, destsize, src, n)`，带安全边界检查（若截断则设置 dest[0]='\0' 并返回错误），面试推荐回答此版本。C++ 中更推荐用 `std::string` 或 `std::string_view` 彻底避免 C 字符串问题。
...
// ⚠️ 以上是经典的 32 位 Linux 简化布局。现代 64 位 Linux + ASLR 下，栈/堆/mmap 区域的起始地址均随机偏移，且栈和 mmap 之间、堆和 mmap 之间布局更复杂。堆的增长方向由 `brk`/`mmap` 共同决定。
...
- `realloc(ptr, size)`：调整内存大小，可能移动位置；ptr=NULL 等价 malloc；size=0 行为由实现定义（可能释放并返回 NULL，也可能返回有效指针，不等价 free）；失败返回 NULL 原内存不释放
- `free(ptr)`：释放堆内存；free(NULL) 安全；不能 double free；free 后建议置 NULL

#### new / delete（C++）

`new` 分配内存并调用构造函数，`delete` 调用析构函数并释放内存。new 失败抛 `std::bad_alloc`（`new(std::nothrow)` 失败返回 `nullptr`），malloc 返回 NULL。new/delete 配对，new[]/delete[] 配对，不可混用。`new` 自动计算大小、类型安全。
...
`#define` 预处理文本替换（无类型检查），`typedef` 编译阶段处理（有类型检查）。// `#define INT_PTR int*` 在 `INT_PTR a, b;` 中 `a` 是 `int*`，`b` 是 `int`（非指针），而 `typedef int* INT_PTR;` 后 `INT_PTR a, b;` 两者都是 `int*`
...
虚函数有默认实现（可被覆盖），纯虚函数仅声明接口（无实现或有函数体，派生类必须重写才能实例化）。虚函数用于"实作继承"，纯虚函数关注接口统一性。
...
`#include <memory>`。C++11: `shared_ptr`（共享拥有/引用计数）、`unique_ptr`（独占拥有/无拷贝语义但有 move）、`weak_ptr`（打破循环引用）。C++98 `auto_ptr` 已弃用（C++11 废弃，C++17 彻底移除，缺乏 move 语义导致容器兼容性问题）。
...
| stack | deque（默认） | 顶部 O(1) | 无序 | 可重复 |
| queue | deque（默认） | 尾插头删 O(1) | 无序 | 可重复 |
...
| 希尔排序 | O(n log²n)（注：希尔排序平均复杂度强烈依赖 gap 序列选择；Hibbard 序列约 O(n^(3/2))，Sedgewick 序列可接近 O(n^(4/3))，不同序列差异显著） | O(n²) | O(1) | 不稳定 |
...
分治法（分解→求解→合并，快排/归并/循环赛日程）；动态规划（重叠子问题+最优子结构，背包/最长公共子序列/编辑距离——注意：斐波那契是计数/求和类问题，不同于优化类DP问题（最短路等），但其子问题结构仍满足DP的重叠子问题特性，可作为DP入门。ACD/面试语境中普遍将其归为DP。严格意义上更贴近"记忆化递归"。）；贪心法（每步最优，最短路径/最小生成树/哈夫曼编码）。
...
管道（无名：亲缘关系单向；有名：任意关系单向（FIFO仅半双工））、信号量（计数器同步）、信号（通知事件）、消息队列（内核链表）、共享内存（最快需同步）、套接字（跨网络）。
...
TCP 三次握手在内核协议栈中完成，与 socket API 的对应关系：
// connect() 触发三次握手（发送 SYN）→ 内核完成握手后 connect() 返回 → 服务端 accept() 从全连接队列取出已完成握手的连接（accept() 返回的是新的已连接 fd，非握手信号）
...
范式：1NF(属性不可分)→2NF(消除部分依赖)→3NF(消除传递依赖)→BCNF(每个非平凡函数依赖的决定因素均为超键)→4NF(消除非平凡多值依赖)。
...
fill | 用 val 填充所有元素
swap | 交换两个 array 内容
get(array) | `std::get<N>(arr)` 获取第 N 个元素引用（编译期索引，N 必须是常量表达式；运行时索引用 `operator[]`）
...
// **重排（随机访问迭代器）**：`random_shuffle`（C++14 废弃 C++17 移除，用 `shuffle` 替代，需配合 `<random>` 随机数引擎如 `std::mt19937`）
...
- vtable 存在只读数据段（.rodata），虚函数调用通过 vptr→vtable 机制实现动态绑定（不依赖 RTTI）；RTTI 仅用于 `dynamic_cast` 运行时类型检查和 `typeid` 获取类型信息。
...
1. **`static_cast`**：相关类型间编译期安全转换（如 int↔float、基类↔派生类指针），可用于向下转型但无运行时检查（由程序员保证类型正确）。
...
构造函数和静态成员函数不能为虚函数。构造函数调用时 vptr 尚未正确设置，无法实现多态；静态成员函数属于类本身，无 this 指针，不参与动态绑定。内联函数**可以**为虚函数——`virtual inline` 合法，但通过基类指针/引用调用虚函数时编译器通常无法内联（去虚化优化后可能例外）。
...
函数参数求值顺序是未指定的（C++17 之前），但与入栈方向无关。x86-64 System V ABI 中前 6 个整型/指针参数通过寄存器传递（rdi, rsi, rdx, rcx, r8, r9），不压栈；多余参数从右向左压栈。Windows x64 前 4 个参数走寄存器（rcx, rdx, r8, r9）。面试应回答"取决于调用约定和 ABI，不可假设压栈顺序"。
...
迭代器是容器与算法间的抽象层：提供更高级的抽象（隐藏底层实现细节）、可移植性、功能扩展（五种迭代器类别支持不同操作集）。注意标准迭代器不保证越界检查（仅调试模式下某些实现提供）。普通指针操作底层内存。
...
TCP头部（20字节+选项）：源端口(16)、目的端口(16)、序列号(32)、确认号(32)、头部长度(4)、保留(3)、控制位(9：NS/CWR/ECE/URG/ACK/PSH/RST/SYN/FIN)、窗口(16)、校验和(16)、紧急指针(16)、选项(可变)。UDP头部（8字节）：源端口(16)、目的端口(16)、长度(16)、校验和(16)。
...
1. 实现简单直观，无需复杂平衡调整 2. 实际性能更稳定（无需旋转和颜色调整） 3. 范围查询更自然（链表结构方便区间遍历） 4. 天然支持并发访问（注意：跳表内存占用通常高于红黑树，其节点需存储多级 forward 指针）
...
| COPY | 创建新表+复制数据+重命名 | 阻塞读写 | 不支持INPLACE的操作 |
...
// 方法一：哈希表 O(N)/O(N)
// 方法二：原地交换（利用值在 [1, n] 内，将 nums[i] 放到 nums[nums[i]-1] 处）O(N)/O(1)
...
// 有限状态自动机（位运算）：ones=(ones^num)&~twos, twos=(twos^num)&~ones
...
### 面试题：什么是 SSO？为什么要存在？各主流编译器的阈值是多少？

**标准答案：**

SSO（Small String Optimization / Short String Optimization）是 `std::string` 的一项关键优化技术。核心思想是：**在 string 对象内部预留一块栈上缓冲区，当字符串长度较小时直接存储在对象内部（避免堆分配），只有超过阈值时才去堆上分配内存。**

为什么需要 SSO：
1. **绝大多数字符串都很短** — 实际工程统计表明，程序中 80%-90% 的字符串长度不超过 20 字符（如 key、name、路径、日志标签等）
2. **避免堆分配开销** — `malloc/new` 的系统调用开销显著，短字符串频繁分配/释放会严重拖慢性能
3. **缓存友好** — 数据与对象本体相邻，减少 cache miss
4. **C++11 起移动语义和 `basic_string` 的连续性要求使得 SSO 成为事实标准** — 标准虽未强制要求 SSO，但三大主流实现全部采用

**各编译器阈值（64 位平台）：**

| 编译器 | SSO 缓冲区大小 | 最大无堆分配长度 | 对象 sizeof |
|--------|---------------|-----------------|------------|
| GCC (libstdc++) | 16 字节 | **15 字符** | 32 字节 |
| Clang (libc++) | 23 字节 | **22 字符** | 24 字节 |
| MSVC (VS 2022) | 16 字节 | **15 字符** | 32 字节 |

> 注：阈值说的是不含 `\0` 的有效字符数，即 `capacity()` 值。如 GCC 中 15 个字符以内（含）不会触发堆分配。

**内存布局 — GCC libstdc++ (32 字节)：**

```cpp
// GCC libstdc++ 的 std::string 核心结构（简化）
struct string_gcc {
    pointer   _M_data;          // 8 bytes — 指向数据的指针
    size_type _M_size;          // 8 bytes — 字符串长度
    union {
        size_type _M_capacity;  // 8 bytes — 分配容量
        value_type _M_buf[16];  // 16 bytes — SSO 本地缓冲区
    };
};
// sizeof = 32
// 短字符串 (_M_capacity < 16)：_M_data 指向 _M_buf
// 长字符串 (_M_capacity >= 16)：_M_data 指向堆上分配的内存
```

**内存布局 — Clang libc++ (24 字节)：**

```cpp
// Clang libc++ 的 std::string 核心结构（简化，64 位）
struct string_clang {
    union {
        struct {
        ...
        value_type _M_buf[24];      // 24 bytes — SSO 缓冲区
    };
    // _M_size 存在缓冲区内
};
// sizeof = 24
// 通过 _M_capacity 的最高 bit 判断：
//   最高 bit = 1 → 短字符串模式，数据在 _M_buf
//   最高 bit = 0 → 长字符串模式，_M_capacity 存容量，数据在堆上
// Clang 将小字符串的末尾 0 作为 size 的 LSByte，紧凑到了极致
```

**面试话术（应答模板）：**

> "SSO 是 std::string 对短字符串的零堆分配优化。string 对象内部通过 union 复用空间：短模式下缓冲区直接存字符数据，`data()` 指向内部缓冲区；长模式下 `data()` 指向堆上内存。GCC 和 MSVC 都是 32 字节对象、15 字符阈值；Clang 压缩到 24 字节对象、22 字符阈值。SSO 本质是用空间换时间——以对象膨胀几个字节为代价，消除短字符串场景下的堆分配延迟。面试中需要能画出内存布局图说明 union 的复用逻辑。"

**追问深层：**

> **问：SSO 对小字符串拼接有什么影响？**
> 答：短字符串拼接可能触发从 SSO 模式切换到堆模式的**模式迁移**——当新长度超过阈值时，需要分配堆内存、拷贝旧数据、释放旧堆内存（如果有）。这是 O(n) 的代价。对于 SSO 阈值附近的反复增长（如循环 `+=`），频繁的模式切换会带来性能抖动。实践中如果预知最终长度，使用 `reserve()` 一次性分配可以避免这种抖动。

> **问：string 的移动构造在 SSO 下会怎样？**
> 答：短字符串（SSO 模式）的移动构造实际上是**拷贝**，因为内部缓冲区紧邻在对象中，无法用指针窃取。长字符串（堆模式）的移动构造才是真正的 O(1) 指针窃取。因此 `std::move(string)` 并非总是零开销——15 字符以内的字符串移动开销等价于拷贝。

---

---

## 异常安全性（Exception Safety）与拷贝交换惯用法

### 面试题：C++ 异常安全性的三个保证级别是什么？如何实现强异常安全？

**标准答案：**

C++ 异常安全性分为三个保证级别，由 Dave Abrahams 在 C++ 标准委员会中总结提出：

| 级别 | 含义 | 说明 |
|------|------|------|
| **基本保证 (Basic)** | 抛出异常后，不泄漏资源、所有对象保持在可析构状态，但数据可能被部分修改 | 最弱保证，但最容易实现 |
| **强烈保证 (Strong)** | 就像 `commit-or-rollback`：操作要么完全成功，要么抛出异常后状态完全不变 | 最常用的高质量保证 |
| **不抛异常保证 (No-throw)** | 承诺绝不抛出异常，总成功 | 最高保证（析构函数、swap、移动构造） |

**实现强异常安全的黄金法则 — Copy-and-Swap 惯用法：**

核心思想：**在"副本"上执行所有可能失败的操作，操作全部成功后通过无抛异常的 swap 替换原对象。**

```cpp
#include <utility>      // std::swap (C++11 起用 <utility>)
...
public:
        ...
        : _data(nullptr), _size(0)
    {
        if (s && *s) {
            _size = std::strlen(s);
            _data = new char[_size + 1];        // 可能抛 std::bad_alloc
            std::strcpy(_data, s);
        }
    }

    // 拷贝构造 — 本身已满足强异常安全（全新对象）
    SafeString(const SafeString& other)
        : _data(nullptr), _size(other._size)
    {
        if (other._data) {
            _data = new char[_size + 1];        // 可能抛异常
            std::strcpy(_data, other._data);    // 不抛
        }
    }

    // ═══════════════════════════════════════════
    // 强异常安全的拷贝赋值 — Copy-and-Swap 惯用法
    // ═══════════════════════════════════════════
    SafeString& operator=(const SafeString& other) {
        // Step 1: 在副本上执行所有可能失败的操作
        SafeString temp(other);   // 如果这里抛异常，*this 完好无损

        // Step 2: 通过不抛异常的 swap 替换原状态
        swap(*this, temp);
        // temp 析构时释放旧资源 — 析构函数不抛异常
    ...
    {
        other._data = nullptr;   // 将源对象置于合法状态
        other._size = 0;
    }

    // 移动赋值 — 同样可以用 copy-and-swap（传值）简化
    SafeString& operator=(SafeString other) noexcept {
        swap(*this, other);
        return *this;
    }
    // ↑ 注意：参数是按值传递的，调用侧已经创建了副本/移动品
    // 这个版本同时处理了拷贝赋值和移动赋值（统一赋值运算符）
...
**Copy-and-Swap 的三个关键要素：**

1. **拷贝构造** 承担所有可能失败的分配操作
2. **swap 必须是 no-throw** — 内置类型和指针的 swap 天然不抛异常
3. **析构函数必须是 no-throw** — RAII 的核心保证（C++11 起析构函数隐式 `noexcept`）

**noexcept 与移动构造的关系：**

这是面试中的高频追问点。`noexcept` 标记对移动构造至关重要，因为它直接影响 STL 容器的行为：

```cpp
// 没有 noexcept 的后果：vector 扩容时无法使用移动！
class Widget {
public:
    Widget(Widget&&) /* 没写 noexcept */;
};
std::vector<Widget> v;
v.push_back(Widget{});   // 扩容时：检测到移动构造不是 noexcept
                          // → 退化为拷贝！性能暴跌！
// 原因：如果移动过程中抛了异常，原数据已被"偷走"，无法回滚
// 而拷贝不会破坏原数据，扩容失败可安全丢弃新内存
// 因此 STL 在移动构造不是 noexcept 时，保守地选择拷贝
```

**正确做法：所有移动构造和移动赋值都应标记 `noexcept`：**
...
> "异常安全有三个级别。基本保证只要不泄漏、对象可析构；强保证要求 commit-or-rollback 语义；no-throw 保证绝不抛异常。实现强保证最经典的方法就是 Copy-and-Swap：先在栈上做一个对象的完整副本（拷贝构造），在副本上完成所有可能失败的操作，最后通过不抛异常的 swap 把新旧状态交换——旧状态随临时对象析构时安全释放。这个惯用法的前提是 swap 不抛异常、拷贝构造能独立安全地完成。另外特别要提的是 noexcept 和移动构造的关系：如果移动构造没有标记 noexcept，STL vector 扩容时会退化为拷贝，造成巨大性能隐患。"
...
### 面试题：什么是 CRTP？为什么用它替代虚函数？请给出完整代码示例。

**标准答案：**

**CRTP**（Curiously Recurring Template Pattern，奇异递归模板模式）是一种 C++ 模板技法：**派生类将自己作为模板参数传给基类模板**，使得基类模板在编译期就能通过 `static_cast` 安全地调用派生类的方法，实现静态多态。

```cpp
// crtp 的典型形式
template <typename Derived>
class Base {
    // Base 通过 static_cast 调用 Derived 的方法
};
...
| 维度 | CRTP（静态多态） | 虚函数（动态多态） |
|------|-----------------|-------------------|
| 绑定时机 | 编译期（模板实例化时） | 运行期（通过 vptr → vtable） |
| 对象大小 | 不含 vptr，更小 | 每个对象多一个 vptr（8 字节） |
| 调用开销 | 直接函数调用，可被内联 | 间接跳转（vtable 查表），难以内联 |
| 类型统一 | 不同派生类是不同类型，不能放入同一容器 | 可通过基类指针/引用放入同一容器 |
| 运行时替换 | 不支持 — 类型在编译期固定 | 支持 — 可动态切换派生类 |
| 代码膨胀 | 每个模板实例产生独立副本 | 虚表共享，代码体积更小 |
...
public:
    // 公共接口 — 在编译期解析调用
    void speak() const {
        // 编译期向下转型：static_cast 是安全的
        // 因为只有正确的 Derived 才能实例化此模板
        static_cast<const Derived*>(this)->speak_impl();
    }

    void eat() const {
        static_cast<const Derived*>(this)->eat_impl();
    }

    // 也可以提供默认实现（覆盖基类的 virtual 函数语义）
    void sleep() const {
...
public:
    ...
    // 如果 Dog 没有定义 sleep_impl，基类的 sleep() 提供默认行为
};
...
// ═══════════════════════════════════════════════
template <typename T>
void make_speak(const Animal<T>& animal) {
    animal.speak();  // 编译期绑定，调用开销 = 0（内联后）
}
...
int main() {
    ...
    dog.speak();     // 直接调用 Dog::speak_impl，可被编译器内联
    cat.speak();     // 直接调用 Cat::speak_impl，可被编译器内联

    make_speak(dog); // 模板展开后 = 直接调用
    make_speak(cat);

    // 注意：Dog 和 Cat 是不同的类型，不能放入同一个
    //       std::vector<Animal*>——这是 CRTP 的取舍
}
```

**std::enable_shared_from_this — 标准库中的 CRTP 经典案例：**
...
public:
    std::shared_ptr<MyClass> get_shared() {
        // shared_from_this() 内部通过 CRTP 机制
        // 找到控制块中的 shared_ptr 实例
        return shared_from_this();
    }

    void do_work() {
        auto self = shared_from_this();  // 安全地获取自身的 shared_ptr
        // 注册回调或传递到异步任务...
    }
};
```

**为什么 `enable_shared_from_this` 必须用 CRTP？**

因为基类 `enable_shared_from_this<T>` 需要知道派生类的完整类型才能返回 `shared_ptr<T>` 而非 `shared_ptr<enable_shared_from_this<T>>`。CRTP 将类型信息在编译期注入基类，避免了运行期 `dynamic_cast` 的开销。

**面试话术：**

> "CRTP 是 C++ 模板元编程的经典技法，派生类把自己作为模板参数传给基类，基类通过 static_cast 在编译期调用派生类方法，实现零开销的静态多态。和虚函数相比，CRTP 消除了 vptr 空间开销和 vtable 间接跳转的时间开销，编译器甚至可以内联整个调用链。代价是 Dog 和 Cat 是不同的类型，不能用同一个基类容器管理所有对象。选型上：运行时需要统一处理多种子类时用虚函数，编译期类型已确定且追求极致性能时用 CRTP。标准库的 enable_shared_from_this 就是用 CRTP 实现的最佳范例。"
...
### 面试题：什么是类型擦除？std::function 怎么实现的？请手动实现一个简化版 any_callable。

**标准答案：**

**类型擦除**是一种 C++ 技术，让一个具体类型的对象"伪装"成满足某个接口的类型，但**不需要继承自某个基类**。擦除的是**编译期的具体类型信息**，保留的是**运行期的行为接口**。这就是所谓的"鸭子类型"——只要满足接口约定即可，无需显式继承。

**std::function 的类型擦除原理：**

`std::function<void(int)>` 可以包装任何满足 `void(int)` 签名的可调用对象——函数指针、lambda、函数对象、成员函数指针——它们完全没有共同的基类。其实现结合了三种技术：

1. **外部多态（External Polymorphism）** — 一个不可见的内部基类 + 模板派生类
2. **桥接模式（Bridge）** — 外壳对象持有指向内部实现的指针（通常是堆分配）
3. **虚函数** — 内部基类定义虚接口，模板子类转发调用

**手动实现简化版 any_callable（类型擦除函数包装器）：**

```cpp
#include <memory>       // unique_ptr
#include <utility>      // forward
#include <iostream>
#include <functional>   // 用 std::function 做对比

// ═══════════════════════════════════════════════
// 简化版 any_callable — 类型擦除的函数包装器
// ═══════════════════════════════════════════════

template <typename Signature>
class any_callable;    // 主模板不定义

template <typename Ret, typename... Args>
class any_callable<Ret(Args...)> {
private:
    // ── 内部基类：定义虚接口（不可见、无模板参数）──
    ...
    struct callable_impl : callable_base {
        ...
        callable_impl(F&& f) : _func(std::forward<F>(f)) {}
        ...
        std::unique_ptr<callable_base> clone() const override {
            return std::make_unique<callable_impl<F>>(_func);
        }
    };

    // ── 外壳对象持有指向内部实现的指针 ──
    std::unique_ptr<callable_base> _impl;

public:
    // 默认构造 — 空包装
    any_callable() = default;

    // 从任意可调用对象构造 — 类型擦除发生在构造函数
    template <typename F>
    any_callable(F&& f)
        : _impl(std::make_unique<callable_impl<std::decay_t<F>>>(
              std::forward<F>(f)))
    {}
    ...
    // 移动赋值
    any_callable& operator=(any_callable&&) = default;

    // 调用运算符
    Ret operator()(Args... args) const {
        if (!_impl) throw std::bad_function_call();
...
struct Multiplier {
    int factor;
    int operator()(int a) const { return a * factor; }
};

int main() {
    any_callable<int(int, int)> f;
    ...
    any_callable<int(int)> g = Multiplier{5}; // 包装函数对象
    std::cout << g(6) << '\n';               // 30

    // 不同类型，同一接口 — 类型擦除成功！
    any_callable<int(int)> h = g;             // 拷贝
    std::cout << h(10) << '\n';              // 50
}
```
...
实际 `std::function` 的实现还会加入 **SBO（Small Buffer Optimization）**——对于不超过一定大小的可调用对象（通常 16-32 字节），直接在 `std::function` 对象内部存储，避免堆分配。GCC libstdc++ 的 `std::function` 默认使用 16 字节内部缓冲区。

**类型擦除的性能开销：**

| 开销来源 | 说明 |
|---------|------|
| 堆分配 | `make_unique` 需要 `new` 一次（除非实现 SBO） |
| 虚函数间接调用 | `invoke()` 是虚函数，无法被内联 |
| 对象内存 | 外壳对象 + `unique_ptr`（8 字节） |

**三种多态方式的对比：**

| 维度 | 虚函数继承 | CRTP 模板 | 类型擦除（std::function） |
|------|----------|----------|-------------------------|
| 是否需要继承 | 是 | 是（模板基类） | 否 |
| 绑定时机 | 运行期 | 编译期 | 运行期 |
| 可否存同一容器 | 是（基类指针） | 否 | 是 |
| 堆分配 | 不额外（对象已在堆） | 无 | 通常有（除非 SBO） |
| 调用开销 | vtable 查表 | 直接 / 内联 | vtable 查表 |
| 扩展新类型 | 需修改类层次 | 需编译实例 | 无需修改任何代码 |

**面试话术：**

> "类型擦除的核心是把'具体类型'擦掉，只保留'行为接口'。std::function 的实现经典地展示了这个技术：内部有一个不可见的虚基类(callable_base)和模板派生类(callable_impl<T>)，构造时把任意可调用对象包裹进模板派生类，之后只通过虚基类指针访问——外部完全不知道 T 是什么。这是用一小点虚函数开销换来了极其灵活的鸭子类型。实际代码中可以用 unique_ptr + 虚函数 + 模板构造函数的组合来实现。最大的性能代价是每次构造需要堆分配（除非做 SBO）以及一次虚函数间接调用。"
...
### 面试题：什么是 PIMPL？它解决了什么问题？请写出完整实现。

**标准答案：**

**PIMPL**（Pointer to IMPLementation，指向实现的指针）是一种 C++ 编译隔离技术：将类的所有**私有成员变量和私有方法的具体细节**移到一个独立的实现类（`Impl`）中，公共类只持有指向 `Impl` 的指针（通常使用 `std::unique_ptr`）。

**核心收益：**

| 收益 | 说明 |
|------|------|
| **减少编译依赖** | 修改私有成员不会触发所有依赖方的重新编译 |
| **ABI 稳定性** | 公共类的大小和布局固定，不因私有成员变化而改变 |
| **隐藏实现细节** | 头文件不暴露第三方库的头文件依赖（如 Boost、protobuf） |
| **加快编译速度** | 头文件变轻量，减少 include 传递 |

**完整代码示例：**

```cpp
// ═══════════════════════════════════════════════════
// widget.h — 公开头文件（轻量、稳定）
// ═══════════════════════════════════════════════════
#pragma once
...
private:
    struct Impl;                      // 只声明，不定义
    std::unique_ptr<Impl> _pImpl;     // 指向实现的唯一指针
};
...
#include <algorithm>
#include <iostream>
// 可以在这里引入任何重量级头文件 — 调用方不受影响
// #include <boost/anything.hpp>
// #include <protobuf/anything.pb.h>

struct Widget::Impl {
    std::string _name;
    std::vector<int> _data;
    // 可以随时添加新成员，不影响 ABI
    int _cache_version = 0;
};
...
{
    _pImpl->_name = std::move(name);
}

// ⚠️ 关键：析构函数必须在 .cpp 中定义
// 原因：析构函数需要看到 Impl 的完整定义才能调用 unique_ptr 的 deleter
// 如果在 .h 中写 ~Widget() = default，编译器会在头文件处实例化
// unique_ptr<Impl> 的析构，此时 Impl 是不完整类型 → 编译错误
Widget::~Widget() = default;   // OK: 此时 Impl 已完整定义
...
// 公共接口：委托给 Impl
void Widget::add_data(int value) {
    _pImpl->_data.push_back(value);
}

std::vector<int> Widget::get_data() const {
    return _pImpl->_data;
}
...
**PIMPL 的特殊成员函数规则（面试高频点）：**

```
规则：析构函数必须在 .cpp 中定义（即使在 .h 中声明为 = default）
原因：unique_ptr 的析构需要完整类型，在 .h 中 Impl 是不完整类型。

规则：如果类需要拷贝，必须在 .cpp 中实现
原因：默认拷贝构造只做浅拷贝 unique_ptr（编译器会直接拒绝，因为 unique_ptr 不可拷贝）
     需要手动实现深拷贝 Impl 的逻辑。

规则：移动构造/赋值可以在 .h 中声明，在 .cpp 中 = default
原因：移动操作只需交换 unique_ptr，但 default 的析构会链式触发
     unique_ptr<Impl> 的析构，跟析构函数同理，必须在 Impl 完整处定义。
```
...
| 代价 | 说明 |
|------|------|
| 一次额外的堆分配 | `make_unique<Impl>()` 需要 `new` |
| 指针间接访问 | 每次访问成员都过一层 `_pImpl->` |
| 代码量增加 | 需要手写转发函数 |
| 调试不便 | 调试器需要多展开一层指针 |

**面试话术：**

> "PIMPL 是 C++ 的编译防火墙技术。在头文件中只声明 Impl 的前向声明并用 unique_ptr 持有，把所有的私有成员塞进 Impl 结构体并定义在 .cpp 中。这样做有三重好处：头文件不引用重量级第三方库，修改私有成员不触发大规模重编译，而且类的二进制布局固定保证 ABI 兼容。实现上最关键的坑是析构函数——必须在 .cpp 中定义或 = default，因为 unique_ptr 析构时需要 Impl 的完整类型。代价是每次构造多一次堆分配和一层指针间接访问。"

---

---

## std::variant 多态 vs 虚函数多态

### 面试题：什么时候用 std::variant 替代虚函数？两者如何选择？

**标准答案：**

C++17 引入的 `std::variant` 提供了一种**封闭式多态**替代方案——当所有可能的子类型在**编译期已知**且数量有限时，用 variant 可以消除虚函数的运行时开销。

**对比总览表：**

| 维度 | std::variant + visit | 虚函数继承 |
|------|---------------------|----------|
| 类型集合 | **封闭** — 编译期固定列表 | **开放** — 可随时添加新子类 |
| 堆分配 | **无** — variant 内存是 union，全在栈上 | 通常有 — 对象通常用 new 创建于堆 |
| 对象大小 | = 最大类型 + 鉴别器（几个字节） | 每个类型独立，基类大小 = vptr |
| 调用开销 | 跳转表 / switch-case（可内联） | vptr → vtable 查表 |
| 缓存局部性 | **好** — 连续内存存储 variant 数组 | 差 — 堆上对象分散 |
| 添加新操作 | 写一个新 visitor（不改已有代码） | 在基类加新虚函数（所有子类都要改） |
| 添加新类型 | 修改 variant 类型列表（所有 visitor 都要改） | 加新子类（不改已有代码） |

这个对比揭示了一个重要的设计抉择，即 **表达式问题 (Expression Problem)**：

```
        | 添加新操作 | 添加新类型
--------|-----------|-----------
虚函数   | 困难(改所有类) | 容易(加新子类)
variant | 容易(加新visitor) | 困难(改所有visitor)
```

**完整代码示例 — 用 variant 实现形状面积计算：**
...
#include <numbers>    // C++20 的 std::numbers::pi
                       // C++17 用 M_PI 或 constexpr double pi = 3.141592...

constexpr double pi = 3.1415926535897932385;

// ═══════════════════════════════════════════════
// 各类型是独立 struct，无需共同基类，无虚函数
// ═══════════════════════════════════════════════
struct Circle {
    double radius;
};
...
// 操作：获取形状名称
// ═══════════════════════════════════════════════
const char* shape_name(const Shape& shape) {
    return std::visit([](const auto& s) -> const char* {
        using T = std::decay_t<decltype(s)>;
...
int main() {
    ...
    for (const auto& s : shapes) {
        total += area(s);              // 无虚函数调用，可用跳转表优化
        std::cout << shape_name(s) << ": "
                  << area(s) << '\n';
    }
...
// class ShapeVirt {
// public:
//     virtual ~ShapeVirt() = default;
//     virtual double area() const = 0;
//     virtual const char* name() const = 0;
// };
// 每个子类需要继承 ShapeVirt；
// 对象必须用指针（unique_ptr<ShapeVirt> 或 shared_ptr<ShapeVirt>）管理；
// 调用 area() 有 vtable 间接跳转；
// 数组存储的是指针，对象散布在堆上。
//
// variant 版本的优势：
//   1. 无堆分配（variant 是值语义，全栈上 / 容器内）
//   2. 无 vptr（每个 Shape 只需 variant 的最大类型大小 + 鉴别器）
//   3. visit 可被编译器优化为跳转表甚至内联
//   4. 连续内存遍历时缓存命中率显著更高
//
// variant 版本的局限：
//   1. 类型列表必须在编译期确定 — 无法动态加载插件类型
//   2. 类型个数会放大 variant 大小 — 不适合几十种以上的封闭类型
//   3. 添加新类型要修改所有 visit — 但添加新操作很容易
```

**性能数据参考（典型基准测试）：**

在遍历 1,000,000 个形状计算面积时：
- `std::variant + visit` 通常比虚函数快 **2x-5x**
- 主要原因：无堆分配碎片 + 连续内存遍历 + 无虚函数间接跳转
- 小对象（< 32 字节）优势尤其明显，因为 variant 消除了分配延迟

**何时使用 variant vs 虚函数 — 决策树：**

```
类型集合在编译期是否完全确定？
├── 是 → 类型数 < 10？对象生命周期是否以值语义为主？
│         ├── 是 → 用 std::variant（性能优、缓存友好）
│         └── 否 → 仍可考虑 variant，但注意二进制大小膨胀
└── 否 → 运行时扩展性更重要？
          ├── 是 → 用虚函数 / 类型擦除
          └── 两者都需要 → std::variant 中嵌套 std::function 做类型擦除
```

**面试话术：**

> "variant 和虚函数是互补的两条路。当所有可能的子类型在编译期已知且数量有限（比如 JSON 的 value 类型、AST 节点类型、协议状态枚举），用 variant + visit 能完全消除虚函数开销——没有 vptr、没有堆分配、没有 vtable 间接跳转，还能享受连续内存遍历的高缓存命中率。核心原理是 variant 用 union 存储值、用 index 做鉴别，visit 展开为编译期的跳转表。但 variant 的软肋是它要求封闭的类型集合——如果需要运行时动态扩展新类型（比如插件系统），还得用虚函数。这个选择本质上就是表达式问题的体现：要容易加操作就用 variant，要容易加类型就用虚函数。"

---
---
# 手写代码高频面试题

> 2025-2026 年社招必考：LRU、shared_ptr、线程池、定时器、内存池、生产者消费者模型是几乎所有大厂的一面/二面手撕代码核心题库。

## LRU Cache 手写实现

### 面试题：实现 LRU Cache，要求 O(1) 完成 get 和 put

**标准答案：** 哈希表 + 双向链表。`unordered_map` 负责 O(1) 按键查找，自定义双向链表负责 O(1) 维护访问时序（最近使用的在头部，最久未使用的在尾部）。
...
public:
    explicit LRUCache(int capacity) : capacity_(capacity), size_(0) {
        head_ = new DLinkedNode(); tail_ = new DLinkedNode();
        head_->next = tail_; tail_->prev = head_;
    }
    ~LRUCache() {
        for (DLinkedNode* cur = head_; cur; ) {
            DLinkedNode* nxt = cur->next; delete cur; cur = nxt;
        }
    }
    ...
    void put(int key, int value) {
        ...
        if (it != cache_.end()) {
            ...
            cache_[key] = node; addToHead(node);
            if (++size_ > capacity_) {
                DLinkedNode* removed = removeTail();
                cache_.erase(removed->key);
                delete removed; --size_;
            }
        }
    }
};
...
**面试话术：**
> "LRU Cache 的核心是双数据结构组合：`unordered_map` 负责 O(1) 按照 key 查找，自定义双向链表负责 O(1) 维护访问时序。四个辅助函数各司其职：addToHead、removeNode、moveToHead、removeTail。哨兵节点消除了头尾插入删除的边界判断。get/put 均为 O(1)，空间 O(capacity)。"

**追问深层：**
- Q: 为什么用自定义链表不用 `std::list`？A: `std::list` 在 map 中需存 `iterator`，而 erase/插入后 iterator 语义复杂；自定义链表还能精确控制内存，展示底层功。但面试时间紧也可以用 `std::list` + `splice`。
- Q: 线程安全怎么做？A: `get`/`put` 入口加 `std::shared_mutex`，读共享锁写独占锁。

---

## 手写 shared_ptr

### 面试题：实现一个简化版 shared_ptr（引用计数智能指针）

**标准答案：** shared_ptr = 原始指针 + 控制块（引用计数）。拷贝构造/赋值计数+1，析构-1 归零时 delete。移动构造转移所有权不增计数。
...
class SharedPtr {
    ...
    void release() {
        if (ctrl_ && --ctrl_->ref_count == 0) { delete ptr_; delete ctrl_; }
    }
public:
    explicit SharedPtr(T* p = nullptr) : ptr_(p), ctrl_(p ? new ControlBlock : nullptr) {}
    SharedPtr(const SharedPtr& o) noexcept : ptr_(o.ptr_), ctrl_(o.ctrl_) { if (ctrl_) ++ctrl_->ref_count; }
    SharedPtr(SharedPtr&& o) noexcept : ptr_(o.ptr_), ctrl_(o.ctrl_) { o.ptr_ = nullptr; o.ctrl_ = nullptr; }
    SharedPtr& operator=(const SharedPtr& o) noexcept {
        if (this != &o) { release(); ptr_ = o.ptr_; ctrl_ = o.ctrl_; if (ctrl_) ++ctrl_->ref_count; }
        return *this;
    }
    ...
    long use_count() const { return ctrl_ ? ctrl_->ref_count.load() : 0; }
...
**面试话术：**
> "shared_ptr 的核心是引用计数。控制块使用 `atomic<long>` 保证多线程下计数的安全性。需要注意的是，引用计数操作是线程安全的，但对 `*ptr_` 的并发访问不提供同步保护（和标准库语义一致）。`make_shared` 将对象和控制块合并为一次分配，减少一次 malloc 和更好的缓存局部性。"

**追问深层：**
- Q: `weak_ptr` 和 `enable_shared_from_this` 原理？A: weak_ptr 持有控制块指针但不增强引用计数；`lock()` 时检查强计数 > 0 则返回 shared_ptr。`enable_shared_from_this` 内部有 `weak_ptr`，通过 CRTP 在编译期注入类型信息。

---

## 手写线程池 (ThreadPool)

### 面试题：实现固定大小线程池，支持返回值获取和优雅关闭
        ...
        for (size_t i = 0; i < n; ++i)
            workers_.emplace_back([this] {
                while (true) {
                    ...
                    { std::unique_lock lk(mtx_); cv_.wait(lk, [this]{ return stop_ || !tasks_.empty(); });
...
**面试话术：**
> "线程池解决两个核心问题：避免频繁创建销毁线程的开销，控制并发资源上限。五要素：工作线程数组、任务队列（类型擦除）、mutex+cv 同步、atomic stop 标志、packaged_task+future 获取返回值。优雅关闭是先置 stop → notify_all → 工作线程检查 `stop_ && tasks_.empty()` 为真才退出（确保剩余任务执行完毕）→ join 等待。"

**追问深层：** Q: 为什么在锁外执行任务？A: 避免长任务阻塞其他线程取任务，也防止任务内部 enqueue 导致死锁。

---

## 手写定时器 (Timer)

### 面试题：设计定时器调度器，支持延迟执行和取消

**标准答案：** 最小堆（`priority_queue` 按截止时间排序）+ 后台线程 + `condition_variable::wait_until`。取消用惰性删除（取消集合），避免 O(n) 堆删除。
...
class Timer {
    ...
    struct Task {
        TimePoint deadline; uint64_t id; std::function<void()> cb;
        bool operator<(const Task& o) const { return deadline > o.deadline; }
    };
    ...
    void run() {
        while (true) {
            ...
            while (!tasks_.empty() && cancelled_.count(tasks_.top().id)) tasks_.pop();
            if (stop_) break;
            if (tasks_.empty()) { cv_.wait(lk, [this]{ return stop_ || !tasks_.empty(); }); continue; }
            auto now = Clock::now();
            if (tasks_.top().deadline <= now) {
                auto cb = std::move(const_cast<Task&>(tasks_.top()).cb);
                tasks_.pop(); lk.unlock(); cb();
            } else {
                cv_.wait_until(lk, tasks_.top().deadline, [this]{
                    return stop_ || tasks_.empty() || cancelled_.count(tasks_.top().id);
                });
            }
        }
    }
...
public:
    Timer() { worker_ = std::thread([this]{ run(); }); }
    ~Timer() { { std::lock_guard lk(mtx_); stop_ = true; } cv_.notify_all(); if (worker_.joinable()) worker_.join(); }

    uint64_t addTimer(int delay_ms, std::function<void()> cb) {
        uint64_t id = next_id_++;
        { std::lock_guard lk(mtx_);
          tasks_.push({Clock::now() + std::chrono::milliseconds(delay_ms), id, std::move(cb)}); }
        cv_.notify_one();
        return id;
    }
    void cancel(uint64_t id) { std::lock_guard lk(mtx_); cancelled_.insert(id); }
};
```

**面试话术：**
> "最小堆是定时器最通用的选择：插入 O(log n)，获取最近任务 O(1)。有序链表插入 O(n) 适合任务极少场景；时间轮（Time Wheel）插入/删除 O(1) 适合海量定时任务（如 Linux 内核定时器）。使用 `steady_clock` 防止系统时间跳变。取消采用惰性删除——被取消任务不立即从堆移除，执行前检查取消集合过滤。"

---

## 手写内存池 (MemoryPool)

### 面试题：实现固定块内存池，减少 malloc/free

**标准答案：** 核心是嵌入式空闲链表——空闲块前几个字节复用为下一块指针，零额外开销。
...
class FixedBlockPool {
    size_t block_size_, blocks_per_chunk_;
    ...
    void expand() {
        ...
        for (size_t i = 0; i < blocks_per_chunk_; ++i) deallocate(chunk + i * block_size_);
    }

public:
    FixedBlockPool(size_t block_sz, size_t per_chunk = 64)
        : block_size_(block_sz < sizeof(void*) ? sizeof(void*) : block_sz)
        , blocks_per_chunk_(per_chunk) {}
    ~FixedBlockPool() { for (auto* c : chunks_) ::operator delete(c); }

    void* allocate() {
        if (!free_list_) expand();
        void* p = free_list_;
        free_list_ = *static_cast<void**>(free_list_);  // 弹出头部
        return p;
    }
    void deallocate(void* p) {
        if (!p) return;
        *static_cast<void**>(p) = free_list_;  // 插入头部
        free_list_ = p;
    }
};
...
public:
    explicit ObjectPool(size_t n = 64) : pool_(sizeof(T), n) {}
    template <typename... Args> T* allocate(Args&&... args) {
        return ::new (pool_.allocate()) T(std::forward<Args>(args)...);
    }
    void deallocate(T* p) { if (p) { p->~T(); pool_.deallocate(p); } }
};
```

**面试话术：**
> "内存池解决频繁 malloc/free 的两个瓶颈：系统调用开销和内存碎片。嵌入式空闲链表将空闲块的前 sizeof(void*) 字节复用为指针，零额外内存。ObjectPool 用 placement new 分离内存管理和对象构造。线程安全方面，推荐 Thread-Local 本地池（类似 jemalloc 的 tcache），只在本地池耗尽时才向全局池申请。"

---

## 生产者消费者模型 (BoundedBlockingQueue)
...
public:
    explicit BoundedBlockingQueue(size_t cap) : cap_(cap), buf_(cap) {}
    void enqueue(T item) {
        std::unique_lock lk(mtx_);
        not_full_.wait(lk, [this]{ return size_ < cap_ || closed_; });
        if (closed_) throw std::runtime_error("closed");
        buf_[tail_] = std::move(item);
        tail_ = (tail_ + 1) % cap_; ++size_;
        not_empty_.notify_one();
    }
    T dequeue() {
        std::unique_lock lk(mtx_);
        not_empty_.wait(lk, [this]{ return size_ > 0 || (closed_ && size_ == 0); });
        if (size_ == 0 && closed_) throw std::runtime_error("closed&empty");
        T item = std::move(buf_[head_]);
        head_ = (head_ + 1) % cap_; --size_;
        not_full_.notify_one();
        return item;
    }
    void close() { { std::lock_guard lk(mtx_); closed_ = true; } not_full_.notify_all(); not_empty_.notify_all(); }
};
```

**面试话术：**
> "环形缓冲区 + 双条件变量是生产者消费者的标准答案。`not_full_` 让生产者等待空间，`not_empty_` 让消费者等待数据——避免惊群效应。关闭时 set close flag → notify_all。SPSC 场景可升级为 lock-free ring buffer（atomic head/tail + CAS），但复杂度高，仅在极致性能场景使用。"
...
> 2025-2026 社招高频：内存序、条件变量陷阱、jthread、协程是字节/腾讯/快手的核心考察点。

## 六种 memory_order 场景对照表

### 面试题：请介绍 C++11 引入的 6 种 memory_order，各自适用什么场景？

**标准答案：**

| memory_order | 语义关键词 | 典型场景 | 代价（相对） |
|---|---|---|---|
| `memory_order_relaxed` | 仅保证原子性，无同步/顺序约束 | 简单计数器、引用计数递增 | 最低 |
| `memory_order_consume` | 数据依赖同步（实际编译器多按 acquire 处理） | 依赖链上的读同步（极少使用） | 理论较低 |
| `memory_order_acquire` | 读端同步：后续读写不能重排到此操作之前 | 读锁、消息消费端 | 中等 |
| `memory_order_release` | 写端同步：先前读写不能重排到此操作之后 | 写锁、消息生产端 | 中等 |
| `memory_order_acq_rel` | 兼具 acquire 和 release 语义 | CAS 操作（如无锁栈入栈） | 较高 |
| `memory_order_seq_cst` | 全局顺序一致性，默认值 | 不确定用哪个时的安全选择 | 最高 |

**代码示例：SPSC 环形缓冲区（使用 acquire/release）**
...
class SPSC_RingBuffer {
    ...
    std::atomic<size_t> write_idx_{0}, read_idx_{0};

public:
    bool push(const T& item) {
        size_t w = write_idx_.load(std::memory_order_relaxed);
        size_t next = (w + 1) & (Capacity - 1);
        size_t r = read_idx_.load(std::memory_order_acquire);  // acquire: 看到消费者的最新位置
        if (next == r) return false;  // 满
        buffer_[w] = item;
        write_idx_.store(next, std::memory_order_release);      // release: 保证 buffer 写入可见
        return true;
    }
    bool pop(T& item) {
        size_t w = write_idx_.load(std::memory_order_acquire);  // acquire: 看到生产者的最新数据
        size_t r = read_idx_.load(std::memory_order_relaxed);
        if (r == w) return false;
        item = buffer_[r];
        size_t next = (r + 1) & (Capacity - 1);
        read_idx_.store(next, std::memory_order_release);        // release: 保证消费可见
        return true;
    }
};
```
...
**面试话术：**
> "六种内存序从弱到强排列。日常记住三个核心组合：relaxed 用于纯粹计数；acquire/release 成对用于生产者-消费者同步，形成 happens-before 关系；seq_cst 是默认值提供全局顺序一致性但代价最高。不熟练时宁可用 seq_cst 确保正确，再用 perf 分析瓶颈逐步降级。"

---

## 条件变量虚假唤醒与信号丢失

### 面试题：什么是条件变量的虚假唤醒和信号丢失？生产环境如何正确处理？

**标准答案：**

**虚假唤醒（Spurious Wakeup）**：线程在条件变量上被唤醒，但实际等待的条件并未满足。POSIX 标准明确允许此行为。
...
void bad_waiter() {
    ...
    if (!ready) { cv.wait(lock); }  // if 而非 while，不检查条件
}
```

**正确写法——谓词循环模式：**
```cpp
// ✅ 正确：带谓词的 wait（C++14 起推荐）
void good_waiter() {
    std::unique_lock<std::mutex> lock(mtx);
    cv.wait(lock, [] { return ready; });  // 等价：while (!ready) cv.wait(lock);
}
```

**信号丢失（Lost Wakeup）**：通知在 wait 之前发送，等待线程永远收不到。解决方案：通知前持有锁修改条件 + wait 中用谓词检查条件（进入内核等待前先检查一次）。
...
public:
    ...
    T pop() {
        ...
        T item = std::move(queue_.front()); queue_.pop();
        lock.unlock();
        not_full_.notify_one();
        return item;
    }
...
**面试话术：**
> "条件变量有两个经典陷阱。虚假唤醒是 OS 允许的行为，必须用 while 循环或带谓词的 wait 反复检查条件。信号丢失发生在通知先于 wait 时——用带谓词的 wait 在进入等待前先检查条件，条件已满足直接返回。推荐先 unlock 再 notify 避免'匆忙等待'。"

---

## std::jthread vs std::thread（C++20）

### 面试题：C++20 的 jthread 与 thread 有哪些区别？

| 特性 | `std::thread` | `std::jthread` |
|---|---|---|
| 析构行为 | 若 joinable() 则 std::terminate() | 自动 request_stop() + join() |
| 中断机制 | 无内置支持 | std::stop_token 协作取消 |
| RAII 安全 | 需手动 join/detach | 天然的 RAII |
| stop_token | 不支持 | get_stop_token() / request_stop() |
...
int main() {
    std::jthread t(worker, 1);  // stop_token 自动传递
    std::this_thread::sleep_for(std::chrono::seconds(1));
    // t 析构时自动 request_stop + join —— 无需手动操作
}
```

**面试话术：**
> "jthread 相比 thread 主要解决两个问题。一是 RAII 安全——析构自动 join，从根本上避免了忘记 join 导致的 terminate 崩溃。二是内置 stop_token 协作式取消机制，替代了以前手动设置 `atomic<bool>` 标志的做法。stoken 可以和 condition_variable_any 无缝配合，还能通过 stop_callback 注册清理逻辑。需要注意的是取消是协作式的，线程必须主动轮询 `stop_requested()`。"
...
> 2025-2026 社招新趋势：字节跳动对 C++20/23 新特性考察权重极高，Concepts/Coroutines/Ranges 是高频题。

## C++20 Concepts 实战

### 面试题：C++20 Concepts 是什么？为什么替代 SFINAE/enable_if？

**标准答案：**

Concepts 是 C++20 引入的编译期约束机制，用于约束模板参数必须满足的条件，在模板实例化之前就进行检查。

**对比：** SFINAE 错误信息几十行模板展开历史难以定位 → Concepts 简洁一行 `constraint not satisfied`；`enable_if` 嵌套晦涩难读 → Concepts 像自然语言描述约束。
...
void sort_container(Container& c) { std::sort(c.begin(), c.end()); }

// 简写语法等价：void sort_container(Sortable auto& c);
// requires 子句等价：template<typename C> requires Sortable<C> void sort_container(C& c);

// 概念重载 —— 根据类型约束选择不同实现
template<typename T> concept Integral = std::is_integral_v<T>;
template<typename T> concept Floating = std::is_floating_point_v<T>;

template<Integral T>    T divide(T a, T b) { return b ? a/b : throw 0; }
template<Floating T>    T divide(T a, T b) { return a / b; }
```

**面试话术：**
> "Concepts 把 SFINAE 的隐式规则变成了显式类型谓词，诊断在重载决议阶段完成而非实例化时。用 `Sortable auto& c` 这样的自然语法约束函数参数，让泛型代码意图更清晰、调试成本更低。"

---

## C++20 Ranges 与 Views

### 面试题：C++20 Ranges 和 Views 解决了什么问题？核心设计理念是什么？
...
// ❌ 传统风格：三次中间容器分配
std::vector<int> traditional(const std::vector<int>& data) {
    ...
    std::transform(evens.begin(), evens.end(), std::back_inserter(squares),
                   [](int x) { return x * x; });
    if (squares.size() > 3) squares.resize(3);
    return squares;  // 三次临时 vector 分配
}

// ✅ C++20 Ranges + Views：零中间容器，惰性求值
auto modern(const std::vector<int>& data) {
    auto result = data
        | std::views::filter([](int x) { return x % 2 == 0; })  // 惰性过滤偶数
        | std::views::transform([](int x) { return x * x; })    // 惰性平方
        | std::views::take(3);                                   // 惰性取前3
    return std::vector<int>(result.begin(), result.end());       // 此时才求值
}

// std::ranges::sort 直接传容器，支持投影
std::ranges::sort(v);                              // 不再需要 begin()/end()
std::ranges::sort(names, {}, &std::string::size);  // 按字符串长度排序（投影）

// 惰性求值的威力：无限序列
auto first5 = std::views::iota(0)
    | std::views::filter([](int x) { return x % 2 == 0; })
    | std::views::transform([](int x) { return x * x; })
    | std::views::take(5);
// 只计算 5 个结果，遍历到第 9 个整数就停了，而非全部
```

**面试话术：**
> "Ranges 核心价值：简化——`std::ranges::sort(vec)` 不用写 begin/end；惰性——`filter | transform | take` 管道不创建中间容器，每个元素逐个流经管道，内存 O(1)；组合——管道操作符从左到右阅读。新项目优先用 `std::ranges::` 版本算法。"

---

## C++20 Coroutines 协程深度 Q&A

### 面试题：C++20 协程的核心机制？和线程、异步回调相比优劣？

**Stackful vs Stackless：**

| 特性 | Stackful（Boost.Coroutine） | Stackless（C++20 Coroutines） |
|---|---|---|
| 栈管理 | 每个协程有独立栈（~1MB） | 无独立栈，状态保存在堆帧中 |
| 内存开销 | 高 | 低（仅保存活跃变量） |
| 挂起点 | 任意嵌套函数中 | 只能在协程函数体顶层 |
| C++ 标准 | 库实现 | C++20 编译器原生支持 |

**核心语法：** 任何包含 `co_await` / `co_yield` / `co_return` 的函数自动成为协程。编译器将函数体切分为多个状态，生成 promise_type（控制中心）、coroutine_handle（帧句柄）、awaitable 协议支持。

**Minimal Generator 示例：**
```cpp
#include <coroutine>
#include <iostream>
...
Generator<int> fibonacci(int n) {
    int a = 0, b = 1;
    for (int i = 0; i < n; ++i) { co_yield a; int tmp = a + b; a = b; b = tmp; }
}
// 使用: for (int val : fibonacci(10)) std::cout << val << ' ';
```
...
| 场景 | 推荐 | 原因 |
|---|---|---|
| CPU 密集型 | 线程/线程池 | 需要真正并行 |
| IO 密集型（大量并发） | 协程 | 避免线程切换开销和内存开销 |
| 少量并发任务 | std::async/jthread | 避免协程复杂性 |
| 生成器/惰性序列 | 协程（co_yield） | 状态自动保存 |
| 需要抢占式调度 | 线程 | 协程是协作式的 |

**面试话术：**
> "C++20 协程是编译器级别的无栈协程，三个关键字：co_await、co_yield、co_return。核心机制是编译器将函数体切分为多个状态，所有活跃变量打包到堆帧中。最大优势是用同步写法表达异步逻辑，适合 IO 密集型高并发（如网络服务器每连接一个协程）。但 C++20 协程只提供底层机制，不像 Python/Go 开箱即用，实际项目常用 cppcoro/folly 等库。"

---

## C++20 Modules

### 面试题：C++20 Modules 解决了什么问题？与 #include 有何本质区别？

**核心对比：**

| 痛点 | `#include` | Modules |
|---|---|---|
| 编译速度 | 每个翻译单元独立展开头文件 | 模块只编译一次，生成 BMI（二进制模块接口） |
| ODR 违规 | 极易出现（同一符号在多处定义） | 模块有清晰边界，编译器强制检查 |
| 宏污染 | 宏不受作用域限制，全局传播 | 宏默认不跨模块传播 |
| 接口控制 | 所有头文件内容完全暴露 | 仅 `export` 的部分可见 |

**基础语法：**
```cpp
// 定义模块接口（math.cppm）
export module math;              // 声明模块
export int add(int a, int b);    // export = 公开接口
// 不 export 的部分是模块私有实现

// 使用模块 (main.cpp)
import math;                     // 导入模块
import <iostream>;               // C++23 头文件单元
```

**面试话术：**
> "Modules 把编译模型从'文本复制'改为'预编译接口'。传统 #include 每个翻译单元都要重新解析头文件→编译时间线性增长。Module 只编译一次生成 BMI，import 时直接读取符号表，编译加速显著。三大优势：编译加速、杜绝 ODR 违规、宏不跨模块泄漏。目前 GCC/Clang/MSVC 三大编译器已基本支持，CMake 3.28+ 有初步支持。"

---

## std::expected（C++23）错误处理模式

### 面试题：C++23 的 std::expected 是什么？相比异常和错误码优势？

**标准答案：** `std::expected<T, E>` 是和类型：要么有预期值 T，要么有错误 E。本质是带错误信息的 optional / 功能性 Either。

**三种方案对比：**

| 维度 | 异常 | 错误码 | std::expected |
|---|---|---|---|
| 正常路径性能 | 零开销 | 每次检查 | 每次检查 |
| 错误路径性能 | 高（栈展开） | 低 | 低 |
| 可组合性 | try/catch 命令式 | if/else 嵌套 | 单子操作（and_then, or_else） |
| 适用场景 | 不可恢复错误 | 系统调用 | 可恢复的业务错误 |
...
auto load_config(const std::string& path) -> std::expected<Config, std::string> {
    return load_file(path)
        .and_then(parse_json)        // 成功时才继续，错误短路传播
        .and_then(validate)
        .transform([](Config c){ c.host = "https://" + c.host; return c; })
        .or_else([](const std::string& err) {  // 失败时恢复
            std::cerr << "Error: " << err << ", using defaults\n";
...
**面试话术：**
> "expected 把错误变成类型系统的一部分——函数签名直接告诉调用者可能失败。和异常比没栈展开代价；和错误码比支持 and_then/transform/or_else 链式组合避免 if/else 嵌套。理解 and_then 是关键——它是 flatMap，只在成功时执行且返回新 expected，错误自动短路。特别适合文件 IO、解析、校验这类多步骤可恢复操作。需要注意的是和 Rust Result 不同，C++ 忽略 expected 返回值不会触发编译警告。"
...
public:
    void pay(double amount) const override { std::cout << "支付宝付款: " << amount << " 元\n"; }
};

class WeChatPay : public PaymentStrategy {
public:
    void pay(double amount) const override { std::cout << "微信付款: " << amount << " 元\n"; }
};
...
public:
    ...
    void executePayment(double amount) const { if (strategy_) strategy_->pay(amount); }
};
```

**面试话术：** "策略模式核心是面向接口编程，把'做什么'和'怎么做'解耦。C++ 中两种实现：虚接口适合策略有内部状态且数量固定；std::function 适合纯行为、可用 lambda 就地定义——`std::sort` 的第三个参数就是典型策略模式。"

---

## 模板方法模式 (Template Method) — NVI 惯用法
...
class DataExporter {
public:
    void exportData() {           // 模板方法 —— 非虚，算法骨架
        connect();
        fetchData();
        if (needTransform()) transformData();
        writeOutput();
        disconnect();
    }
    virtual ~DataExporter() = default;
private:
    virtual void connect()    { std::cout << "[默认] 连接\n"; }
    virtual void fetchData()  = 0;
    virtual void transformData() = 0;
    virtual void writeOutput() = 0;
    virtual void disconnect() { std::cout << "[默认] 断开\n"; }
    virtual bool needTransform() const { return true; }  // 钩子方法
};
```

**面试话术：** "经典场景——单元测试框架的 setUp/tearDown/runTest 就是模板方法。NVI 惯用法让基类在调用虚函数前后加锁/日志/校验等不变逻辑。和策略模式区别：模板方法用继承固定骨架，策略用组合替换整体算法。"
...
| 维度 | 简单工厂 | 工厂方法 | 抽象工厂 |
|------|----------|----------|----------|
| 类数 | 1 个工厂类 | 1 抽象 Creator + N 个 ConcreteCreator | 1 抽象 Factory + N 个 ConcreteFactory |
| 扩展方式 | 修改工厂类 switch（违反开闭原则） | 新增 ConcreteCreator | 新增 ConcreteFactory |
| 创建对象 | 单个产品 | 单个产品 | 产品族（多关联产品） |
| 适用场景 | 产品类型少且稳定 | 产品经常扩展 | 需要多系列产品协同 |

**工厂方法代码：**
```cpp
class Transport { public: virtual void deliver() const = 0; virtual ~Transport() = default; };
class Truck : public Transport { void deliver() const override { std::cout << "公路运输\n"; } };
class Ship : public Transport { void deliver() const override { std::cout << "海运\n"; } };

class Logistics {
public:
    void planDelivery() { auto t = createTransport(); t->deliver(); }
...
class NewsAgency {
    std::vector<std::weak_ptr<Observer>> observers_;  // ★ weak_ptr 防循环引用
public:
    void attach(std::shared_ptr<Observer> obs) { observers_.push_back(obs); }
    void detach(std::shared_ptr<Observer> obs) {
        observers_.erase(std::remove_if(observers_.begin(), observers_.end(),
            [&](const auto& wp) { auto sp = wp.lock(); return sp && sp == obs; }), observers_.end());
    }
    void notify(const std::string& msg) {
        observers_.erase(std::remove_if(observers_.begin(), observers_.end(),
            [](const auto& wp) { return wp.expired(); }), observers_.end());
        for (auto& wp : observers_)
            if (auto obs = wp.lock()) obs->update(msg);  // lock() 检查存活
    }
};
...
class User : public Observer {
    std::string name_;
public:
    User(std::string n) : name_(std::move(n)) {}
    void update(const std::string& msg) override { std::cout << "[" << name_ << "] " << msg << "\n"; }
};
```

**面试话术：** "C++ 观察者用 `weak_ptr` 而非裸指针或 shared_ptr。shared_ptr 会导致循环引用永久泄漏；裸指针有野指针风险。`weak_ptr.lock()` 既检查存活又临时提升引用计数防止通知中途析构。"
...
# 排查三步骤：
(gdb) info threads                     # 1. 查看所有线程
(gdb) thread apply all bt              # 2. 打印所有线程调用栈 → 定位卡在哪个 lock
# 关键输出：Thread 2 等待 lockB (持有 lockA)，Thread 3 等待 lockA (持有 lockB)
(gdb) thread 2                         # 3. 切到等待线程
(gdb) frame 2
(gdb) p m_to.mtx.__owner               # 查看锁持有者 PID → 12347 即 Thread 3
# 结论：Thread 2 等 Thread 3 的锁，Thread 3 等 Thread 2 的锁 → 死锁
```

**面试话术：** "死锁排查三步法：`info threads` 看多少线程在等锁；`thread apply all bt` 定位每个线程卡在哪；切到对应 frame 用 `p mutex.__owner` 找出持有者 PID 画出持有-等待图。预防方案：用 `std::lock` 同时锁定多个互斥量，或规定锁获取顺序一致。"
...
```bash
perf stat -d ./my_program          # 宏观健康度：IPC、缓存命中率、分支预测
perf record -g ./my_program        # 微观热点：采样 CPU cycles
perf report                        # 交互式查看热点函数排序
perf script | stackcollapse-perf.pl | flamegraph.pl > flame.svg  # 火焰图
```

关键指标：`cache-misses` > 1% 需关注、`branch-misses` > 1% 需关注、IPC < 1 说明流水线停顿。火焰图横轴宽度 = CPU 时间占比，"平顶山" = 热点函数需优化。

**面试话术：** "`perf stat` 回答'宏观健康度'——一次看 IPC、缓存命中率，快速定位瓶颈在 CPU 还是内存。`perf record` 回答'具体谁在耗'——采样找到哪个函数花了最多 CPU cycles。如果无法解析 C++ 符号，检查是否编译了 `-g -fno-omit-frame-pointer`。"
...
|---:|------|:---:|
| `-O0` | 无优化 | ✓ 最佳 |
| `-O2` | 工业级：指令调度、CSE、循环展开、自动内联（Linux 内核用此） | 部分变量被优化 |
| `-O3` | 激进：-O2 + 更激进内联 + 循环向量化 | 大量变量消失 |
| `-Os` | 大小优先 | — |

**LTO（链接时优化）：** 编译时保留中间表示，链接时全局优化（跨文件内联、死代码消除、常量传播），可带来 5-15% 性能提升。编译命令加 `-flto`。

**`-march=native`：** 为当前 CPU 生成最优指令（AVX2/AVX-512/BMI2），但要求构建机器 = 部署机器。

**UB 陷阱：** 数组越界、strict aliasing 违规、无锁编程内存序错误在 `-O2` 下可能被优化掉。
...
64 位长整型：`[1bit 未使用] [41bit 毫秒时间戳] [10bit (5bit datacenter ID + 5bit worker ID)] [12bit 序列号]`

```cpp
class Snowflake {
    uint64_t nextId() {
        uint64_t ts = nowMs();
        if (ts < lastTs_) throw "clock rollback";
        if (ts == lastTs_) {
            seq_ = (seq_ + 1) & 0xFFF;
            if (seq_ == 0) ts = waitNextMs(lastTs_);
        } else seq_ = 0;
        lastTs_ = ts;
        return ((ts - epoch_) << 22) | (datacenterId_ << 17) | (workerId_ << 12) | seq_;
    }
};
```

**面试话术：** "Snowflake 核心价值：本地生成、趋势递增、全局唯一。UUID v4 是 string 无序 → B+ 树主键插入随机 IO；Snowflake int64 有序 → 减少页分裂。时钟回拨是最大挑战——一般用本地记录最大时间戳兜底，或 ZK 持久化 workerId 加时钟校验。"
...
**核心结论**：P 必须接受 → 实际是 CP vs AP 抉择。ZooKeeper/etcd（CP）—— 分区时拒绝写入；Eureka/Cassandra（AP）—— 分区时各节点正常服务最终一致。

---

## 一致性哈希 vs Redis Hash Slot

一致性哈希：数据映射到 0~2³²-1 的环上，顺时针找节点。增减节点时仅相邻节点数据迁移。需虚拟节点解决数据倾斜。

Redis Cluster 用 16384 个哈希槽而非一致性哈希环：槽位图仅 2KB，通过 gossip 协议节点间高效同步；`CRC16(key) % 16384` 直接定位节点无需代理。

---

## 分布式锁 (Redlock)

单实例 Redis：`SET lock_key unique_value NX PX 30000` → 释放时 Lua 脚本校验 unique_value。

Redlock 算法：向 N 个独立 Redis 节点依次请求锁，过半成功 + 耗时 < TTL 才算获取成功。

**争议（Martin Kleppmann）**：时钟跳跃和 GC 停顿可能导致锁过早失效。解决方案：fencing token——每次锁操作带单调递增 token，存储层拒绝旧 token 的写入。正确性要求的分布式锁建议用 ZooKeeper/etcd。
...
### 面试题：请讲一下 InnoDB 的 MVCC 实现原理

**标准答案：**

**三隐藏列**：`DB_TRX_ID`（6字节，最后修改的事务ID）、`DB_ROLL_PTR`（7字节，指向 undo log 的回滚指针）、`DB_ROW_ID`（无主键时自动生成）。

**undo log 版本链**：每次 UPDATE/DELETE，InnoDB 将旧版本通过 `DB_ROLL_PTR` 串成单向链表。insert undo log 提交即删除；update undo log 需等无 ReadView 引用时才由 purge 线程清理。

**ReadView 快照读**：事务创建 ReadView 包含 `m_ids`（活跃事务ID集合）、`min_trx_id`、`max_trx_id`、`creator_trx_id`。可见性规则：`trx_id == creator` 可见（自修改）→ `trx_id < min` 可见（已提交）→ `trx_id >= max` 不可见（future）→ `trx_id` 在 `m_ids` 中不可见（活跃未提交）。

**RC vs RR 核心差异**：RC 每次快照读生成新 ReadView（能读到已提交更新，不可重复读）。RR 只在首次快照读生成 ReadView 并复用（快照一致，解决不可重复读；配合间隙锁解决幻读）。

**面试话术：** "MVCC 核心思想是以空间换时间，用多版本避免锁竞争。RC 和 RR 的本质区别仅仅在于 ReadView 生成的时机——RC 每次新生成，RR 只生成一次复用。两个易忽略细节：update undo log 由后台 purge 线程异步清理，长事务会导致 undo log 膨胀；RR 隔离级别下间隙锁与 MVCC 互补——MVCC 解决读一致，间隙锁解决写冲突。"

---

## MySQL 慢查询优化策略

**JOIN 优化**：小表驱动大表 → EXPLAIN 第一行是驱动表 → 被驱动表 JOIN 列必须有索引。

**LIMIT 深分页优化**：`LIMIT 1000000,20` 需扫描前 100 万行丢弃。
- 延迟关联：`SELECT * FROM t JOIN (SELECT id FROM t ORDER BY time LIMIT 1000000,20) AS tmp USING(id)`
- 游标分页：`WHERE id > last_id LIMIT 20`（不能跳页）
- 覆盖索引：先取主键 ID 再回表

**COUNT(*) 优化**：InnoDB 无行数缓存需遍历索引 → 优先用二级索引（叶子更小）；非精确用 `SHOW TABLE STATUS` 的 Rows（误差 40-50%）；或用 Redis 维护计数值。

**覆盖索引**：查询所有列都在索引中（Extra 显示 `Using index`），避免回表随机 I/O。

---

## Redis 数据结构编码阈值

| 类型 | 编码（小数据） | 编码（大数据） | 转换阈值 |
|------|-------------|-------------|---------|
| String | embstr（≤ 44 字节） | raw | Redis 5.0+ 44 字节（redisObject 16 字节 + SDS hdr 3 字节，64 字节 jemalloc 桶） |
| Hash | ziplist/listpack | hashtable | entries > 512 或 value > 64 字节 |
| ZSet | ziplist/listpack | skiplist + dict | entries > 128 或 value > 64 字节 |
| List | quicklist（3.2+ 统一） | — | 每个节点 8KB（-2 配置） |
| Set | intset | hashtable | entries > 512 或出现非整数 |

**面试话术：** "核心思想是时空权衡——数据小用紧凑结构省内存，数据大用高效结构保性能。三个易错点：embstr 阈值在 Redis 4.0 是 39 字节（redisObject 16→20）；intset 转 hashtable 不可逆；Redis 7.0 将 ziplist 全面替换为 listpack 消除级联更新问题。"
...
| 问题 | 触发条件 | 核心区别 | 关键解法 |
|------|----------|----------|----------|
| **穿透** | 查询不存在的数据（DB 也无） | 数据永不存在，绕过缓存打 DB | 布隆过滤器 + 空值缓存 + 参数校验 |
| **击穿** | 热点 key 过期瞬间，大量请求打 DB | 单个热点 key 失效 | 互斥锁（SETNX）重建 + 逻辑过期 |
| **雪崩** | 大量 key 同时过期 或 Redis 宕机 | 批量 key 集体失效 | 过期加随机值 + 多级缓存 + 熔断限流 |
...
Product getProduct(Long id) {
    ...
    if (p != null) return p;
    bool locked = redis.setnx("lock:product:" + id, "1", 10, SECONDS);
    if (locked) {
        try {
            p = db.query(id);
            cache.set("product:" + id, p, 3600);
        } finally { redis.del("lock:product:" + id); }
    } else {
        sleep(50); return getProduct(id);  // 等待后重试
    }
    return p;
}
```
...
**Cache-Aside（最常用）**：读——先查缓存 miss 则查 DB 回写缓存。写——先更新 DB 再删除缓存（非更新缓存！更新缓存可能双写时序问题）。

**为什么先写 DB 后删缓存？** 先删缓存再写 DB：A 删缓存 → B 读 miss 查 DB 得旧值 → B 写旧值到缓存 → A 写 DB。缓存脏数据无过期前一直存在。

**删缓存失败怎么办？** 同步重试 + MQ 异步重试 + Canal binlog 订阅做补偿 + TTL 兜底。

**面试话术：** "缓存一致性本质是 CAP 权衡，追求最终一致性。首选 Cache-Aside + 先写 DB 后删缓存 + TTL。对一致性要求高可加 Canal binlog 异步补偿。真正强一致场景（金融账户）不走缓存直接读主库。"

---

## 数据库分库分表策略

**垂直拆分**：按业务模块分库（用户/订单/商品库），或按冷热字段分表。

**水平分片**：单表 > 1000 万行或 > 10GB 时考虑。分片键选择原则：高区分度、覆盖核心查询 WHERE、关联表同分片键（避免跨分片 JOIN）。

**常见问题**：
- 跨分片 JOIN → 反范式化冗余字段或应用层聚合
- 全局唯一 ID → Snowflake / Leaf 方案
- 分片扩容 → 预留充足分片数，用一致性哈希原理降低迁移量
- 中间件 → ShardingSphere（客户端模式，无中心节点）

---

## MySQL 索引优化补充

**索引条件下推（ICP）**：MySQL 5.6+，存储引擎在回表前用索引列过滤 WHERE 条件，减少不必要的回表 I/O。Extra 显示 `Using index condition`。覆盖索引优先于 ICP（若已 `Using index` 则无需 ICP）。

**多范围读（MRR）**：将二级索引回表随机 I/O 转为按主键顺序的顺序 I/O。HDD 效果显著，SSD 也有提升。

**自适应哈希索引（AHI）**：InnoDB 自动为热点页建立哈希索引（等值查询 O(1)）。高并发下 AHI 读写锁可能成为瓶颈。`innodb_adaptive_hash_index = ON`（默认）。

**Filesort 优化**：尽量通过联合索引消除 filesort（索引顺序与排序方向一致）。否则单路排序（4.1+）：排序字段 + SELECT 列一起读入排序缓冲区一次完成。

**面试话术：** "ICP 是'能少查就少查'（回表前过滤），MRR 是'随机转顺序'（按主键排序批量回表），AHI 是'常用常快'（自动哈希缓存热点页）。实战：Extra 出现 `Using index condition` 说明 ICP 在起效但未完全消除过滤；`Using index` 覆盖索引是最优状态。`sort_buffer_size` 不是越大越好——每个排序线程分配一个，并发场景可能 OOM。"
