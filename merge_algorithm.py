...
def remove_lcr_section(lines):
    """Remove the LCR section (lines 26596-37192)."""
    # LCR starts at line 26596 (0-indexed: 26595)
    # LCR ends before line 37193 (0-indexed: 37192)
    # 丑数 combined section starts at line 37193 (0-indexed: 37192)
    # 丑数 combined section ends before line 37516 (0-indexed: 37515)
    ...
    if lcr_start is not None and 丑数_end is not None:
        # Remove LCR section and 丑数 section
...
def remove_overlapping_leetcode(lines):
    ...
    while i < len(lines):
        ...
        # Check if this is a LeetCode problem header
        match = re.match(r'^## (\d+)\. (.+)$', line)
        if match:
            problem_num = match.group(1)
            problem_title = match.group(2).strip()

            # Check if this problem overlaps with 剑指Offer
            problem_key = f"{problem_num}. {problem_title}"
            if problem_key in overlapping_leetcode:
                # Skip this problem and all its content until the next problem or section
                print(f"Removing overlapping LeetCode problem: {problem_key}")
                removed_count += 1
                i += 1
                # Skip until we find the next problem header or section
                while i < len(lines) and not (
                    lines[i].startswith('## ') and
                    (re.match(r'^## \d+\.', lines[i]) or lines[i].startswith('## 剑指 Offer'))
                ):
                    i += 1
                continue
...
def main():
    ...
    lines = remove_lcr_section(lines)
    print(f"Line count after removing LCR: {len(lines)}")
